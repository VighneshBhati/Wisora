...
export const ImageQuestionExtractor: React.FC<ImageQuestionExtractorProps> = ({
  ...
  const callGeminiWithImage = async (base64Image: string): Promise<ExtractedQuestion[]> => {
    ...
    // For now, we'll use text-only mode since OpenRouter doesn't support image input in the same way
    // You might need to use a different approach for image processing with OpenRouter
    const messages: AIMessage[] = [
      { role: 'user', content: `${userPrompt}\n\nNote: This is a text-based extraction. Please provide sample questions based on the context.` }
    ];
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && 'questions' in parsed && Array.isArray((parsed as { questions: unknown }).questions)) {
        return (parsed as { questions: unknown[] }).questions.map((q: unknown, index: number) => {
          const question = q as Record<string, unknown>;
          return {
            id: (question.id as string) || `img_${Date.now()}_${index}`,
            question_text: (question.question_text as string) || '',
            question_type: (question.question_type as 'mcq' | 'written') || 'written',
      ...
      throw new Error(t('quizEditor.imageExtraction.apiError', { 
        status: 'N/A', 
