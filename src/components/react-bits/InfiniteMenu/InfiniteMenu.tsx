  ...
  private initTexture(): void {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        // "cover" crop — fill the cell, center horizontally, bias toward top (face area)
        const imgW = img.naturalWidth || img.width;
        const imgH = img.naturalHeight || img.height;
        const scale = Math.max(cellSize / imgW, cellSize / imgH);
        const drawW = imgW * scale;
        const drawH = imgH * scale;
        // center X, bias Y to top-third so faces are always visible
        const offsetX = x + (cellSize - drawW) / 2;
        const offsetY = y + (cellSize - drawH) * 0.25;
